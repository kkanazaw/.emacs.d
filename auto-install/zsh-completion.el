;;; zsh-completion.el --- ZSH completion for the shell buffer
...
;; shell-command prompts that are based on zsh completion.
;;
...
;;
;; Zsh completion for emacs:
;; - is aware of zsh builtins, aliases and functions
;; - does file expansion inside of colon-separated variables
;;   and after redirections (> or <)
;; - escapes special characters when expanding file names
;; - is configurable through programmable zsh completion
;;
;; When the first completion is requested in shell model or a shell
;; command, zsh-completion.el starts a separate zsh
;; process.  Zsh-completion.el then uses this process to do the actual
;; completion and includes it into Emacs completion suggestions.
;;
;; A simpler and more complete alternative to zsh-completion.el is to
;; run a zsh shell in a buffer in term mode(M-x `ansi-term').
...
;; INSTALLATION
;;
;; 1. copy zsh-completion.el into a directory that's on Emacs load-path
;; 2. add this into your .emacs file:
;;   (autoload 'zsh-completion-dynamic-complete \"zsh-completion\"
;;     \"ZSH completion hook\")
;;   (add-hook 'shell-dynamic-complete-functions
;; 	'zsh-completion-dynamic-complete)
...
;; launches a new zsh process.
;;
;; You'll get better results if you turn on programmable zsh completion.
;; On Ubuntu, this means running:
;;   sudo apt-get install zsh-completion
;; and then adding this to your .zshrc:
;;   . /etc/zsh_completion
;;
;; Right after enabling programmable zsh completion, and whenever you
;; make changes to you .zshrc, call `zsh-completion-reset' to make
;; sure zsh completion takes your new settings into account.
;;
;; Loading /etc/zsh_completion often takes time, and is not necessary
;; in shell mode, since completion is done by a separate process, not
;; the process shell-mode process.
;;
;; To turn off zsh completion when running from emacs but keep it on
;; for processes started by zsh-completion.el, add this to your .zshrc:
;; if [[ ( -z "$INSIDE_EMACS" || "$EMACS_ZSH_COMPLETE" = "t" ) &&\
;;      -f /etc/zsh_completion ]]; then
;;   . /etc/zsh_completion
;; fi
;;
;; Emacs sets the environment variable INSIDE_EMACS to the processes
;; started from it. Processes started by zsh-completion.el have
;; the environment variable EMACS_ZSH_COMPLETE set to t.
;;
;; CAVEATS
;;
;; Using a separate process for doing the completion has several
;; important disadvantages:
;; - zsh completion is slower than standard emacs completion
;; - the first completion can take a long time, since a new zsh process
;;   needs to be started and initialized
;; - the separate process is not aware of any changes made to zsh
;;   in the current buffer.
;;   In a standard terminal, you could do:
;;     $ alias myalias=ls
;;     $ myal<TAB>
;;   and zsh would propose the new alias.
;;   Zsh-completion.el cannot do that, as it is not aware of anything
;;   configured in the current shell. To make zsh-completion.el aware
;;   of a new alias, you need to add it to .zshrc and restart the
;;   completion process using `zsh-completion-reset'.
;;
;; COMPATIBILITY
;;
;; zsh-completion.el is quite sensitive to the OS and ZSH version.
...
;;
;; and using the following zsh versions:
;;   ZSH 3.2.17
;;   ZSH 3.2.32
;;   ZSH 3.2.39
;;
;; zsh-completion.el does not works on XEmacs.
;;
...
;;
;; * zsh-completion-require-process: set MAILCHECK to -1
;; to disable mail check message. 
;;
;; 2009-08-01   Stephane Zermatten <szermatt@gmail.com>
;;
;; * zsh-completion-generate-line: add missing compgen 
...
;;; ---------- Customization
(defgroup zsh-completion nil
  "ZSH configurable command-line completion "
  :group 'shell
  :group 'shell-command)

(defcustom zsh-completion-enabled t
  "Enable/Disable ZSH configurable command-line completion globally.

This flag is useful for temporarily disabling zsh completion
once it's been installed.

Setting this variable to t is NOT enough to enable ZSH completion.
ZSH completion is only available in the environment for which
`zsh-completion-dynamic-complete' has been registered. See
`zsh-completion-setup' for that."
  :type '(boolean)
...
should point to a recent version of ZSH (ZSH 3) with support
for command-line completion."
  :type '(file :must-match t)
  :group 'zsh-completion)

(defcustom zsh-completion-process-timeout 2.5
  "Timeout value to apply when waiting from an answer from zsh.
If zsh takes longer than that to answer, the answer will be
ignored."
  :type '(float)
  :group 'zsh-completion)

(defcustom zsh-completion-initial-timeout 30
  "Timeout value to apply when talking to zsh for the first time.
The first thing zsh is supposed to do is process /etc/zsh_complete,
which typically takes a long time."
  :type '(float)
  :group 'zsh-completion)

(defcustom zsh-completion-nospace nil
  "Never let zsh add a final space at the end of a completion.

When there is only one completion candidate, zsh sometimes adds
...
to remove the extra space zsh adds after a completion."
  :type '(boolean)
  :group 'zsh-completion)

;;; ---------- Internal variables and constants

(defvar zsh-completion-process nil
  "Zsh process object.")
(defvar zsh-completion-alist nil
  "Maps from command name to the 'complete' arguments.

For example if the following completion is defined in zsh:
  complete -F _cdargs_aliases cdb
the following entry is added to `zsh-completion-alist':
 (\"cdb\" . (\"-F\" \"_cdargs\"))

See `zsh-completion-add-to-alist'.")

(defconst zsh-completion-wordbreaks-str "\"'@><=;|&(:"
...
(defconst zsh-completion-wordbreaks
  (append zsh-completion-wordbreaks-str nil)
  "`zsh-completion-wordbreaks-str' as a list of characters.")

;;; ---------- Functions: completion

;;;###autoload
(defun zsh-completion-setup ()
  "Register zsh completion for the shell buffer and shell command line.

This function adds `zsh-completion-dynamic-complete' to the completion
...
zsh completion in your .emacs file because it forces you to load the module
...
;;;###autoload
(defun zsh-completion-dynamic-complete ()
  "Complete word at cursor using ZSH completion.
...
completion.  Return nil if no match was found."
  (when zsh-completion-enabled
    (when (not (window-minibuffer-p))
      (message "Zsh completion..."))
    (let* ( (start (comint-line-beginning-position))
	    (pos (point))
	    (tokens (zsh-completion-tokenize start pos))
	    (open-quote (zsh-completion-tokenize-open-quote tokens))
	    (parsed (zsh-completion-process-tokens tokens pos))
	    (line (cdr (assq 'line parsed)))
	    (point (cdr (assq 'point parsed)))
	    (cword (cdr (assq 'cword parsed)))
	    (words (cdr (assq 'words parsed)))
	    (stub (nth cword words))
	    (completions (zsh-completion-comm line point words cword open-quote))
	    ;; Override configuration for comint-dynamic-simple-complete.
	    ;; Zsh adds a space suffix automatically.
	    (comint-completion-addsuffix nil) )
      (if completions
	  (comint-dynamic-simple-complete stub completions)
	;; no standard completion
	;; try default (file) completion after a wordbreak
	(zsh-completion-dynamic-try-wordbreak-complete stub open-quote)))))

(defun zsh-completion-dynamic-try-wordbreak-complete (stub open-quote)
...
`zsh-completion-dynamic-complete'."
  (let* ((wordbreak-split (zsh-completion-last-wordbreak-split stub))
...
(defun zsh-completion-join (words)
...
(defun zsh-completion-quote (word)
...
(defun zsh-completion-parse-line (start pos)
  "Split a command line in the current buffer between START and POS.

This function combines `zsh-completion-tokenize' and
`zsh-completion-process-tokens'.  It takes the same arguments as
`zsh-completion-tokenize' and returns the same value as
`zsh-completion-process-tokens'."
  (zsh-completion-process-tokens
   (zsh-completion-tokenize start pos) pos))

(defun zsh-completion-process-tokens (tokens pos)
  "Process a command line split into TOKENS that end at POS.

This function takes a list of tokens built by
`zsh-completion-tokenize' and returns the variables compgen
...
Return an association list with the current symbol as keys:
 ...
 cword - 0-based index of the word to be completed in words (number)"
  (zsh-completion-parse-line-postprocess
   (zsh-completion-parse-current-command tokens) pos))

(defun zsh-completion-parse-line-postprocess (tokens pos)
  "Extract from TOKENS the data needed by compgen functions.

This function takes a list of TOKENS created by `zsh-completion-tokenize'
for the current buffer and generate the data needed by compgen functions
as returned by `zsh-completion-parse-line' given the cursor position POS."
  (let* ((first-token (car tokens))
	 (last-token (car (last tokens)))
	 (start (or (car (zsh-completion-tokenize-get-range first-token)) pos))
	 (end (or (cdr (zsh-completion-tokenize-get-range last-token)) pos))
	 (words (zsh-completion-strings-from-tokens tokens)))
...
(defun zsh-completion-parse-current-command (tokens)
  "Extract from TOKENS the tokens forming the current command.

This function takes a list of TOKENS created by
`zsh-completion-tokenize' for the current buffer and select the
...
Return a sublist of TOKENS."
  (nreverse
   (catch 'zsh-completion-return
     (let ((command nil) (state 'initial))
       (dolist (token tokens)
	 (let* (
		(string (zsh-completion-tokenize-get-str token))
		(is-terminal
		 (and (member string '(";" "&" "|" "&&" "||"))
		      (let ((range (zsh-completion-tokenize-get-range token)))
			(= (- (cdr range) (car range))
			   (length string))))))
	   (cond
	    (is-terminal
...
(defun zsh-completion-strings-from-tokens (tokens)
  "Extract the strings from TOKENS.

This function takes all strings from TOKENS and retrun it as a
list of strings.

TOKENS should be in the format returned by `zsh-completion-tokenize'."
  (mapcar 'zsh-completion-tokenize-get-str tokens))

(defsubst zsh-completion-tokenize-get-range (token)
  "Return the TOKEN range as a cons: (start . end)."
  (cdr (assq 'range token)))

(defsubst zsh-completion-tokenize-set-end (token)
  "Set the end position of TOKEN to the cursor position."
  (setcdr (zsh-completion-tokenize-get-range token) (point)))

(defsubst zsh-completion-tokenize-append-str (token str)
  "Append to TOKEN the string STR."
  (let ((str-cons (assq 'str token)))
    (setcdr str-cons (concat (cdr str-cons) str))))

(defsubst zsh-completion-tokenize-get-str (token)
  "Return the TOKEN string."
  (cdr (assq 'str token)))

(defsubst zsh-completion-tokenize-open-quote (tokens)
  "Return the quote character that was still open in the last token.

TOKENS is a list of token as returned by
`zsh-completion-tokenize'."
  (cdr (assq 'quote (car (last tokens)))))

(defun zsh-completion-tokenize (start end)
  "Tokenize the portion of the current buffer between START and END.

This function splits a ZSH command line into tokens.  It knows
...
`zsh-completion-tokenize-get-str'.  It can be modified using
`zsh-completion-tokenize-append-str'.
...
`zsh-completion-tokenize-get-range'. The end position can be
set using `zsh-completion-tokenize-set-end'.
...
The current format of a token is '(string . (start . end))."
  (save-excursion
    (goto-char start)
    (nreverse (zsh-completion-tokenize-new-element end nil))))

(defun zsh-completion-tokenize-new-element (end tokens)
  "Tokenize the rest of the line until END and complete TOKENS.

This function is meant to be called exclusively from
`zsh-completion-tokenize' and `zsh-completion-tokenize-0'.
...
END prepended to it."
  (skip-chars-forward " \t\n\r" end)
  (if (< (point) end)
      (zsh-completion-tokenize-0 end tokens
				  (list
				   (cons 'str "")
				   (cons 'range (cons (point) nil))))
    tokens))

(defun zsh-completion-tokenize-0 (end tokens token)
  "Tokenize the rest of the token until END and add it into TOKENS.

This function is meant to be called exclusively from
`zsh-completion-tokenize-new-element'.
...
calls `zsh-completion-tokenize-1'.
...
Return TOKENS with new tokens prepended to it."
  (let ( (char-start (char-after))
    ...
    (zsh-completion-tokenize-1 end quote tokens token)))

(defun zsh-completion-tokenize-1 (end quote tokens token)
  "Tokenize the rest of the token.

This function is meant to be called exclusively from
`zsh-completion-tokenize-0'.

This function tokenize the rest of the token and either call
itself and `zsh-completion-tokenize-0' recursively or append the
token to the list of token and call
`zsh-completion-tokenize-new-element' to look for the next
token.
...
Return TOKENS with new tokens prepended to it."
  ...
  (let ((local-start (point)))
    (when (= (skip-chars-forward "[;&|]" end) 0)
      (skip-chars-forward (zsh-completion-nonsep quote) end))
    (zsh-completion-tokenize-append-str
     token
  ...
  (cond
   ...
   ((and (char-before) (= ?\\ (char-before)))
    (forward-char)
    (let ((str (zsh-completion-tokenize-get-str token)))
      (aset str (1- (length str)) (char-before)))
    (zsh-completion-tokenize-1 end quote tokens token))
   ;; opening quote
   ((and (not quote) (char-after) (or (= ?' (char-after)) (= ?\" (char-after))))
    (zsh-completion-tokenize-0 end tokens token))
   ;; closing quote
   ...
   ((and quote (char-after) (not (= quote (char-after))))
    (forward-char)
    (zsh-completion-tokenize-append-str token (char-to-string (char-before)))
    (zsh-completion-tokenize-1 end quote tokens token))
   ;; word end
   (t
    (zsh-completion-tokenize-set-end token)
    (when quote
      (push (cons 'quote quote) token))
    (push token tokens)
    (zsh-completion-tokenize-new-element end tokens))))

(defconst zsh-completion-nonsep-alist
  '((nil . "^ \t\n\r;&|'\"#")
    (?' . "^ \t\n\r'")
    (?\" . "^ \t\n\r\""))
...
quote).  Get it using `zsh-completion-nonsep'.")

(defun zsh-completion-nonsep (quote)
  "Return the set of non-breaking characters when QUOTE is the current quote.

QUOTE should be nil, ?' or ?\"."
  (cdr (assq quote zsh-completion-nonsep-alist)))

;;; ---------- Functions: getting candidates from zsh

(defun zsh-completion-comm (line pos words cword open-quote)
  "Set LINE, POS, WORDS and CWORD, call compgen, return the result.

This function starts a separate zsh process if necessary, sets
...
The result is a list of candidates, which might be empty."
  ;; start process now, to make sure zsh-completion-alist is
  ;; set before we run zsh-completion-generate-line
  (zsh-completion-require-process)
  (zsh-completion-send
   (concat
    (zsh-completion-generate-line line pos words cword)
    " 2>/dev/null"))
  (zsh-completion-extract-candidates (nth cword words) open-quote))

(defun zsh-completion-extract-candidates (stub open-quote)
...
to directory names, merging STUB with the result.  See `zsh-completion-fix'
for more details."
  (let ((zsh-completion-prefix stub)
	(zsh-completion-open-quote open-quote))
    (mapcar 'zsh-completion-fix
	    (with-current-buffer (zsh-completion-buffer)
	      (split-string (buffer-string) "\n" t)))))

(defun zsh-completion-fix (str &optional prefix open-quote)
...
nil, the value of `zsh-completion-prefix' is used.  This allows
...
`zsh-completion-open-quote' is used.  This allows
calling this function from `mapcar'.
...
It should be invoked with the comint buffer as the current buffer
for directory name detection to work."
  (let ((prefix (or prefix zsh-completion-prefix))
	(open-quote (or open-quote (and (boundp 'zsh-completion-open-quote) zsh-completion-open-quote)))
	(suffix ""))
    (zsh-completion-addsuffix
     (let* ((rebuilt)
	    (rest (cond
		   ((zsh-completion-starts-with str prefix)
		    (substring str (length prefix)))
		   ;; zsh expands the home directory automatically. This is confusing
		   ;; for comint-dynamic-simple-complete
		   ((and (zsh-completion-starts-with prefix "~")
			 (zsh-completion-starts-with str (expand-file-name "~")))
		    (substring (concat "~" (substring str (length (expand-file-name "~"))))
			       (length prefix)))
		   ((zsh-completion-starts-with prefix str)
		   ...
		   ;; this feature.
		   ((zsh-completion-starts-with
		     (setq rebuilt (concat (zsh-completion-before-last-wordbreak prefix) str))
		     prefix)
		   ...
		   ;; hope for the best.
		   (t str))))
       (when (zsh-completion-ends-with rest " ")
	 (setq rest (substring rest 0 -1))
	 (unless zsh-completion-nospace
	     (setq suffix " ")))
       (concat prefix (zsh-completion-escape-candidate rest open-quote) suffix)))))

(defun zsh-completion-escape-candidate (completion-candidate open-quote)
  "Escapes COMPLETION-CANDIDATE.

This function escapes all special characters in the result of
zsh completion.  It does nothing if COMPLETION-CANDIDATE looks
...
(defconst zsh-completion-known-suffixes-regexp
  (concat "[" (regexp-quote zsh-completion-wordbreaks-str) "/ ]$")
  "Regexp matching known suffixes for `zsh-completion-addsuffix'.")

(defun zsh-completion-addsuffix (str)
...
\(concat STR \"/\"). Otherwise it retruns STR."
  (if (and (null (string-match zsh-completion-known-suffixes-regexp str))
	   (file-accessible-directory-p (expand-file-name str default-directory)))
	(concat str "/")
    str))

(defun zsh-completion-before-last-wordbreak (str)
...
Wordbreaks characters are defined in 'zsh-completion-wordbreak'."
  (car (zsh-completion-last-wordbreak-split str)))

(defun zsh-completion-after-last-wordbreak (str)
...
Wordbreaks characters are defined in 'zsh-completion-wordbreak'."
  (cdr (zsh-completion-last-wordbreak-split str)))

(defun zsh-completion-last-wordbreak-split (str)
...
Return a CONS containing (before . after)."
  (catch 'zsh-completion-return
    (let ((end (- (length str) 1)))
      (while (>= end 0)
	(when (memq (aref str end) zsh-completion-wordbreaks)
	  (throw 'zsh-completion-return (cons (substring str 0 (1+ end)) (substring str (1+ end)))))
	(setq end (1- end))))
      (cons "" str)))

(defun zsh-completion-ends-with (str suffix)
...
(defun zsh-completion-starts-with (str prefix)
...
;;; ---------- Functions: zsh subprocess

(defun zsh-completion-require-process ()
  "Return the zsh completion process or start it.

If a zsh completion process is already running, return it.

Otherwise, create a zsh completion process and return the
result.  This can take a since zsh needs to start completely
before this function returns to be sure everything has been
initialized correctly.

The process uses `zsh-completion-prog' to figure out the path to
zsh on the current system.
...
the first file found in the following list are sourced if they exist:
 ~/.emacs_zsh.sh
 ~/.emacs.d/init_zsh.sh
Or, to be more exact, ~/.emacs_$(basename `zsh-completion-prog').sh)
and ~/.emacs.d/init_$(basename `zsh-completion-prog').sh)

To allow scripts to tell the difference between shells launched
by zsh-completion, the environment variable EMACS_ZSH_COMPLETE
is set to t."
  (if (zsh-completion-is-running)
      zsh-completion-process
    ;; start process
    (let ((process) (oldterm (getenv "TERM")))
      (unwind-protect
	  (progn
	    (setenv "EMACS_ZSH_COMPLETE" "t")
	    (setenv "TERM" "dumb")
	    ;; (setq process
	    ;; 	  (start-process
	    ;; 	   "*zsh-completion*"
	    ...
	    (zsh-completion-send "PS1='\v'" process zsh-completion-initial-timeout)
	    (zsh-completion-send "function __zsh_complete_wrapper { eval $__ZSH_COMPLETE_WRAPPER; }" process)
	...
	;; finally
	(progn
	  (setenv "EMACS_ZSH_COMPLETE" nil)
	  (setenv "TERM" oldterm)
	  (when process
	    (condition-case err
		;;(zsh-completion-kill process)
	      (error nil))))))))

(defun zsh-completion-cd-command-prefix ()
  "Build a command-line that CD to default-directory.

Return a zsh command-line for going to default-directory or \"\"."
  (if default-directory
      (concat "cd >/dev/null 2>&1 "
	      (zsh-completion-quote (expand-file-name default-directory))
	      " ; ")
    ""))

(defun zsh-completion-build-alist (buffer)
  "Build `zsh-completion-alist' with the content of BUFFER.

BUFFER should contains the output of:
  complete -p

Return `zsh-completion-alist', which is slightly parsed version
of the output of \"complete -p\"."
  (with-current-buffer buffer
    (save-excursion
      (setq zsh-completion-alist nil)
      (goto-char (point-max))
      (while (= 0 (forward-line -1))
	(zsh-completion-add-to-alist
	 (zsh-completion-strings-from-tokens
	  (zsh-completion-tokenize
	   (line-beginning-position)
	   (line-end-position)))))))
  zsh-completion-alist)

(defun zsh-completion-add-to-alist (words)
  "Add split 'complete' line WORDS to `zsh-completion-add-to-alist'.
...
Return `zsh-completion-alist'."
  (when (string= "complete" (car words))
	...
	(push (cons command options) zsh-completion-alist))))
  zsh-completion-alist)

(defun zsh-completion-generate-line (line pos words cword)
  "Generate a command-line that calls compgen.

This function looks into `zsh-completion-alist' for a matching compgen
argument set. If it finds one, it executes it. Otherwise, it executes the
default zsh completion (compgen -o default)
...
arguments will be passed to this function or command as:
 ...
 COMP_WORDS, taken from WORDS (a zsh array)
 COMP_CWORD, taken for CWORD

Return a zsh command-line that calls compgen to get the completion
candidates."
  (concat
   (zsh-completion-cd-command-prefix)
   (let* ( (command-name (file-name-nondirectory (car words)))
     ...
     (cond
      ...
      ((not compgen-args)
       ...
       (zsh-completion-join (list "compgen" "-o" "default" stub)))
...
(defun zsh-completion-reset ()
  "Force the next completion command to start with a fresh ZSH process.

This function kills any existing ZSH completion process.  This way, the
next time ZSH completion is requested, a new process will be created with
the latest configuration.

Call this method if you have updated your .zshrc or any zsh init scripts
and would like zsh completion in Emacs to take these changes into account."
  (interactive)
  (zsh-completion-kill zsh-completion-process)
  (setq zsh-completion-process nil))

(defun zsh-completion-kill (process)
...
(defun zsh-completion-buffer ()
  "Return the buffer of the ZSH process, create the ZSH process if necessary."
  (process-buffer (zsh-completion-require-process)))

(defun zsh-completion-is-running ()
  "Check whether the zsh completion process is running."
  (and zsh-completion-process (eq 'run (process-status zsh-completion-process))))

(defun zsh-completion-send (commandline &optional process timeout)
  "Send a command to the zsh completion process.

COMMANDLINE should be a zsh command, without the final newline.

PROCESS should be the zsh process, if nil this function calls
`zsh-completion-require-process' which might start a new process.

TIMEOUT is the timeout value for this operation, if nil the value of
`zsh-completion-process-timeout' is used.

Once this command has run without errors, you will find the result
of the command in the zsh completion process buffer."
  ;;(message commandline)
  (let ((process (or process (zsh-completion-require-process)))
	(timeout (or timeout zsh-completion-process-timeout)))
	...
	(unless (accept-process-output process timeout)
	  (error "Timeout while waiting for an answer from zsh-completion process")))
      (goto-char (point-max))
